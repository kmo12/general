...
def create_calendar(
    ...
    keyboard.add(
        ...
        # InlineKeyboardButton(
        #     "Закрыть", callback_data=calendar_callback.new("CANCEL", year, month, "!")
        # ),
        InlineKeyboardButton(
            "Сегодня", callback_data=calendar_callback.new("TODAY", year, month, "!")
        ),
        InlineKeyboardButton(
...
def callback_inline_handler(
        ...
        month: int = False,
        day: int = False,
        uniq_calendar: CallbackData = False):
    """
    ...
    if not uniq_calendar:
        raise Exception("Did not give 'uniq_calendar' parameter in callback_inline_handler()")
    # At this point, we are sure that this calendar is ours. So we cut the line by the separator of our calendar
    name, action, year, month, day = call.data.split(uniq_calendar.sep)
    # Processing the calendar. Get either the date or None if the buttons are of a different type
    date = calendar_query_handler(
        bot=bot, call=call, name=name, action=action, year=year, month=month, day=day
    )
    # There are additional steps. Let's say if the date DAY is selected, you can execute your code. I sent a message.
    if action == "DAY" or action == "TODAY":
